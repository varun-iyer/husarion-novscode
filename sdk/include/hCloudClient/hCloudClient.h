...
struct Resource {
	...
	__attribute__((deprecated)) Resource(Type tag, const char* data): tag(tag), data(data) {}
	__attribute__((deprecated)) Resource(const Resource& other) = default;
};
...
public:
	...
	/**
	 * Loads HTML template at given URL.
	 */
	void loadHtml(const char* res);

	__attribute__((deprecated)) void loadHtml(Resource res);
