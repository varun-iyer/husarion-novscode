...
public:
	...
	/**
	 * @brief Regulate angle velocity of the motor.
	 * @param rpm revolutions per minute
	 * @param power a power that is attached to a motor, the higher the value, the shorter time to reach final speed (values from 0 to 1000).
	 * @param block 1 - if you want to block the task until final speed is reached.
	 * @return 1 if order has been successfully stored in motor queue
	 */
	// bool go(int16_t rpm, uint16_t power = 1000, bool block = 0);

	/**
	 * @brief Set a power attached to a motor.
	 * @param power a power that is attached to a motor, the higher the value, the more power is attached to a motor (values from -1000 to 1000).
	 * @param block 1 - if you want to block the task until power is set.
	 * @return 1 if order has been successfully stored in motor queue
	 */
